...
import { getImageFetchCorsHint } from './lib/imageApiShared'
...
export function getErrorToastMessage(message: string): string {
  ...
  if (!text) return t('operationFailed')
  ...
  if (firstLine.length > ERROR_TOAST_MAX_LENGTH) return t('operationFailedDetail')
  return firstLine || t('operationFailed')
}
...
function getTimeoutStreamingHint(profile?: TimeoutStreamingHintProfile | null) {
  ...
  if (profile.streamImages !== true) return t('timeoutStreamingHint')
  if (partialImages === 0) return t('timeoutPartialZeroHint')
  return partialImages < 3 ? t('timeoutPartialLowHint') : ''
}

function createOpenAITimeoutError(timeoutSeconds: number, profile?: TimeoutStreamingHintProfile | null) {
  return t('openaiTimeoutError', { seconds: timeoutSeconds, hint: getTimeoutStreamingHint(profile) })
}
      ...
      setAppMode: (appMode) => {
        ...
        if (settings.agentApiConfigMode === 'off' && activeProfile.provider === 'openai' && activeProfile.apiMode !== 'responses') {
          state.setConfirmDialog({
            title: t('needResponsesApiTitle'),
            message: t('needResponsesApiMsg', { name: activeProfile.name }),
        ...
        if (settings.agentApiConfigMode !== 'off') {
          state.setConfirmDialog({
            title: t('agentApiConfigIncompleteTitle'),
            message: t('agentApiConfigIncompleteMsg', { msg: agentValidationError.message }),
            confirmText: t('goToSettings'),
            cancelText: t('cancel'),
            action: () => {
        ...
        state.setConfirmDialog({
          title: t('providerNoAgentTitle'),
          message: t('providerNoAgentMsg', { name: activeProfile.name }),
          confirmText: t('goToSettings'),
          cancelText: t('cancel'),
          action: () => {
            useStore.getState().setShowSettings(true, 'api')
...
function scheduleOpenAIWatchdog(taskId: string, timeoutSeconds: number, profile?: TimeoutStreamingHintProfile | null) {
  ...
  const timer = setTimeout(() => {
    ...
    if (failed) useStore.getState().showToast(t('openaiTaskTimeout'), 'error')
  }, remainingMs)
...
export function showCodexCliPrompt(force = false, reason = t('codexPromptRewritten')) {
  const state = useStore.getState()
  const settings = state.settings
  const promptKey = getCodexCliPromptKey(settings)
  if (!force && (settings.codexCli || state.dismissedCodexCliPrompts.includes(promptKey))) return
  const promptRewriteGuardMessage = settings.allowPromptRewrite
    ? t('codexCliAllowRewrite')
    : t('codexCliGuardRewrite')

  state.setConfirmDialog({
    title: t('codexCliDetectedTitle'),
    message: t('codexCliDetectedMsg', { reason, guard: promptRewriteGuardMessage }),
    confirmText: t('codexCliEnable'),
    action: () => {
      const state = useStore.getState()
...
function getAgentProfileValidationError(settings: AppSettings): { profile: ApiProfile | null; message: string } | null {
  ...
  if (!textProfile || textProfile.provider !== 'openai' || textProfile.apiMode !== 'responses') {
    return { profile: textProfile, message: t('agentNeedResponsesModel') }
  }
  const textProfileError = validateApiProfile(textProfile)
  if (textProfileError) return { profile: textProfile, message: t('textModelConfigIncomplete', { msg: textProfileError }) }

  if (normalized.agentApiConfigMode === 'hybrid') {
    const imageProfile = getAgentImageApiProfile(normalized)
    if (!imageProfile) return { profile: null, message: t('imageModelConfigMissing') }
    const imageProfileError = validateApiProfile(imageProfile)
    if (imageProfileError) return { profile: imageProfile, message: t('imageModelConfigIncomplete', { msg: imageProfileError }) }
  }
...
function getTaskApiProfileName(task: TaskRecord) {
  return task.apiProfileName || task.apiModel || t('unknownConfig')
}
...
function getApiRequestNetworkErrorHint(
  ...
  if (elapsedSeconds <= 15) {
    if (usesApiProxy) {
      return t('networkHintProxy')
    }
    const unsupportedApiHint = profile?.provider === 'openai'
      ? t('apiNotSupportMode', { mode: getApiModeApiName(profile.apiMode) })
      : ''
    return t('networkHintImmediate', { unsupported: unsupportedApiHint })
  }

  if (elapsedSeconds >= 55 && elapsedSeconds <= 75) {
    return t('networkHint60', { hint: getTimeoutStreamingHint(profile) })
  }

  if (elapsedSeconds >= 110 && elapsedSeconds <= 140) {
    return t('networkHint120', { hint: getTimeoutStreamingHint(profile) })
  }

  return t('networkHintGeneral', { hint: getTimeoutStreamingHint(profile) })
}
...
async function completeRecoveredFalTask(task: TaskRecord, result: Awaited<ReturnType<typeof getFalQueuedImageResult>>) {
  ...
  useStore.getState().showToast(t('falTaskRecovered', { n: outputIds.length }), 'success')
  if (!isAgentTask(task)) showTaskCompletionNotification(t('imageGenerationDone'), t('falTaskRecovered', { n: outputIds.length }))
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  if (normalizedSettings.reuseTaskApiProfileTemporarily && (reusedTaskApiProfileId || reusedTaskApiProfileMissing)) {
    ...
    if (!reusedProfile) {
      if (options.useCurrentApiProfileWhenReusedMissing) {
        ...
        setConfirmDialog({
          title: t('apiConfigNotFoundTitle'),
      message: t('apiConfigNotFoundMsg', { taskProfile: reusedTaskApiProfileName || t('unknownConfig'), currentProfile: activeProfile.name }),
      confirmText: t('useCurrentConfigSubmit'),
      cancelText: t('abandonSubmit'),
      action: () => {
  ...
  if (validateApiProfile(activeProfile)) {
    showToast(t('completeRequestApiConfig', { msg: validateApiProfile(activeProfile)! }), 'error')
  ...
  if (maskDraft) {
    try {
      ...
      if (coverage === 'full' && !options.allowFullMask) {
        setConfirmDialog({
          title: t('confirmEditWholeImageTitle'),
          message: t('confirmEditWholeImageMsg'),
          confirmText: t('continueSubmit'),
          tone: 'warning',
          action: () => {
  ...
  await putTask(task)
  useStore.getState().showToast(t('taskSubmitted'), 'success')
...
function createAgentAbortError() {
  return new DOMException(t('agentRequestStopped'), 'AbortError')
}
...
export function stopAgentResponse(conversationId = useStore.getState().activeAgentConversationId) {
  ...
  if (controller) {
    ...
    if (markAgentRoundStopped(conversationId, runningRound.id)) {
      useStore.getState().showToast(t('stoppedGenerating'), 'info')
    }
    return
  }
...
async function continueRecoveredAgentRound(taskId: string) {
  ...
  try {
    ...
    if (agentValidationError) {
      failRound(t('cannotContinueRecover', { msg: agentValidationError.message }))
      return
    }
    ...
    if (!activeProfile || !imageProfile) {
      failRound(t('agentApiConfigMissingRecover'))
      return
    }
...
export async function submitAgentMessage() {
  ...
  const agentValidationError = getAgentProfileValidationError(normalizedSettings)
  if (agentValidationError) {
    showToast(t('completeAgentApiConfig', { msg: agentValidationError.message }), 'error')
  ...
  if (conversation.rounds.some((round) => round.status === 'running')) {
    showToast(t('waitForGeneration'), 'info')
    return
  }
...
export async function regenerateAgentAssistantMessage(conversationId: string, roundId: string) {
  ...
  if (!conversation || !sourceRound || !sourceUserMessage) {
    showToast(t('agentMessageNotFound'), 'error')
    return
  }
...
async function executeAgentRound(
  ...
  try {
    ...
    if (round.maskImageId && !maskDataUrl) throw new Error(t('maskImageMissingError'))
    ...
    const pauseAgentImageTaskForRecovery = (toolCallId: string, err: unknown) => {
      ...
      if (latestTask.apiProvider === 'fal' && latestTask.falRequestId && latestTask.falEndpoint) {
        useStore.getState().setTaskStreamPreview(taskId)
        updateTaskInStore(taskId, {
          ...createTaskErrorPatch(latestTask, t('falConnectionLost'), Date.now()),
          falRecoverable: true,
        })
        scheduleFalRecovery(taskId)
        return true
      ...
      if (latestTask.customTaskId) {
        useStore.getState().setTaskStreamPreview(taskId)
        updateTaskInStore(taskId, {
          ...createTaskErrorPatch(latestTask, t('customConnectionLost'), Date.now()),
    ...
    const callHybridImageApiSingle = async (opts: {
      ...
      return {
        ...
        error: result.failedRequests?.[0]?.error ?? (dataUrl ? null : t('apiNoImageData')),
    ...
    markAgentRoundTasksFailed(
      conversationId,
      roundId,
      requestSettings.agentApiConfigMode === 'hybrid' ? t('customToolNoImage') : t('builtinToolNoImage'),
      undefined,
    ...
    const limitNotice = reachedToolLimit ? t('toolLimitReached', { n: maxToolCalls }) : ''
    const joinedText = textSegments.join('\n\n').trim()
    const finalContent = [joinedText, limitNotice]
      .filter(Boolean)
      .join(joinedText ? '\n\n' : '')
      || (taskIds.length > 0 || outputIds.length > 0 ? t('imageGenerated') : '')
    ...
    useStore.getState().showToast(outputIds.length > 0 ? t('agentGeneratedImages') : t('agentReplied'), 'success')
    showTaskCompletionNotification(
      outputIds.length > 0 ? t('agentGeneratedImages') : t('agentReplied'),
      outputIds.length > 0 ? t('agentReplyDoneWithImages', { n: outputIds.length }) : t('agentReplyDone'),
    )
  } catch (err) {
    ...
    if (networkErrorHint && !message.includes(getImageFetchCorsHint())) {
      message += `\n${networkErrorHint}`
    }
    ...
    useStore.getState().showToast(t('agentRequestFailed', { msg: message }), 'error')
  } finally {
...
async function executeTask(taskId: string) {
  ...
  if (!taskProfile && task.apiProfileId) {
    updateTaskInStore(taskId, {
      ...createTaskErrorPatch(task, t('taskApiConfigMissing'), Date.now()),
      falRecoverable: false,
      customRecoverable: false,
    })
    return
  ...
  try {
    ...
    for (const imgId of task.inputImageIds) {
      ...
      if (!dataUrl) throw new Error(t('inputImageMissingError'))
      inputDataUrls.push(dataUrl)
    }
    let maskDataUrl: string | undefined
    if (task.maskImageId) {
      maskDataUrl = await ensureImageCached(task.maskImageId)
      if (!maskDataUrl) throw new Error(t('maskImageMissingError'))
    }
    ...
    if (taskProvider === 'openai' && activeProfile.apiMode === 'responses' && !activeProfile.codexCli) {
      if (promptWasRevised) {
        ...
        showCodexCliPrompt(false, t('promptMissingInfo'))
      }
    }
    ...
    const completionMessage = failedCount > 0
      ? t('generationCompletePartial', { ok: outputIds.length, fail: failedCount })
      : t('generationCompleteCount', { n: outputIds.length })
    useStore.getState().showToast(completionMessage, failedCount > 0 ? 'error' : 'success')
    if (!isAgentTask(task)) showTaskCompletionNotification(t('imageGenerationDone'), `${completionMessage}。`)
    ...
    if (latestTask.apiProvider === 'fal' && latestFalRequestInfo && isNetworkRecoverableError(err)) {
      updateTaskInStore(taskId, {
        ...createTaskErrorPatch(task, t('falConnectionLost'), Date.now()),
      ...
      updateTaskInStore(taskId, {
        ...createTaskErrorPatch(task, t('customConnectionLost'), Date.now()),
      ...
      if (networkErrorHint && !errorMessage.includes(getImageFetchCorsHint())) {
        errorMessage += `\n${networkErrorHint}`
      }
      updateTaskInStore(taskId, {
...
export function createFavoriteCollection(name: string) {
  ...
  if (Array.from(normalizedName).length > 60) {
    useStore.getState().showToast(t('collectionNameTooLong'), 'error')
    return null
  }
  const state = useStore.getState()
  ...
  state.showToast(t('collectionCreated', { name: normalizedName }), 'success')
  return collection
}
...
export function renameFavoriteCollection(collectionId: string, name: string) {
  ...
  showToast(t('collectionRenamed'), 'success')
}
...
export async function updateTasksFavoriteCollections(taskIds: string[], collectionIds: string[]) {
  ...
  clearSelection()
  showToast(ids.length ? t('collectionUpdated') : t('unfavorited'), 'success')
}
...
export async function deleteFavoriteCollection(collectionId: string, deleteTasks = false) {
  ...
  useStore.getState().showToast(t('collectionDeleted', { name: collection.name }), 'success')
}
...
export async function reuseConfig(task: TaskRecord) {
  ...
  if (missingReusedProfile) {
    setConfirmDialog({
      title: t('apiConfigNotFoundTitle'),
      message: t('apiConfigNotFoundMsg', { taskProfile: taskProfileName, currentProfile: currentProfile.name }),
      confirmText: t('useCurrentConfigSubmit'),
      cancelText: t('abandonSubmit'),
      action: () => {
  ...
  showToast(
    shouldTemporarilyReuseProfile && matchedProfile
      ? t('profileTemporarilyReused', { name: matchedProfile.name })
      : t('configReusedToInput'),
    'success',
  )
}
...
export async function editOutputs(task: TaskRecord) {
  ...
  showToast(t('outputsAddedToInput', { n: added }), 'success')
}
...
export async function removeMultipleTasks(taskIds: string[]) {
  ...
  useStore.getState().showToast(t('tasksDeleted', { n: deletedCount }), 'success')
}
...
export async function clearFailedTasks(taskIds?: string[]) {
  ...
  if (partialFailedTaskIds.size) {
    ...
    showToast(t('partialFailedCleared', { n: partialFailedTaskIds.size }), 'success')
  }
}
...
export async function removeTask(task: TaskRecord) {
  ...
  if (deletedCount === 0) return
  useStore.getState().showToast(t('taskDeleted'), 'success')
}
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  ...
  showToast(t('selectedDataCleared'), 'success')
}
...
async function completeRecoveredCustomTask(task: TaskRecord, result: Awaited<ReturnType<typeof getCustomQueuedImageResult>>) {
  ...
  useStore.getState().showToast(t('customAsyncTaskRecovered', { n: outputIds.length }), 'success')
  if (!isAgentTask(task)) showTaskCompletionNotification(t('imageGenerationDone'), t('customAsyncTaskRecovered', { n: outputIds.length }))
  else void continueRecoveredAgentRound(task.id)
}
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    const state = useStore.getState()
    if (options.exportTasks && hasActiveDataOperations(state.tasks, state.agentConversations)) throw new Error(t('exportBusy'))
    ...
    useStore.getState().showToast(plan.length > 1 ? t('zipDownloadRequested', { n: plan.length }) : t('dataExported'), 'success')
  } catch (e) {
    console.error('exportData failed', e)
    const detail = e instanceof Error ? e.message.trim() : String(e).trim()
    useStore.getState().showToast(detail ? t('exportFailedWithDetail', { detail }) : t('exportFailedUnknown'), 'error')
  }
}
...
export async function importData(input: File | File[], options: ImportOptions = { importConfig: true, importTasks: true }): Promise<boolean> {
  try {
    const state = useStore.getState()
    if (options.importTasks && hasActiveDataOperations(state.tasks, state.agentConversations)) throw new Error(t('importBusy'))
    const files = Array.isArray(input) ? input : [input]
    if (!files.length) throw new Error(t('noBackupSelected'))
    ...
    if (multipart) {
      if (selected.some((part) => !part.manifest.backupPart)) throw new Error(t('cannotMixBackup'))
      ...
      if (!validSet || indexes.size !== selected.length) throw new Error(t('invalidParts'))
      if (options.importTasks && (selected.length !== first.total || indexes.size !== first.total)) {
        throw new Error(t('incompleteParts', { total: first.total }))
      }
    ...
    if (options.importConfig && !options.importTasks && !settingsManifests.length) throw new Error(t('backupNoConfig'))
    ...
    let msg = t('dataImported')
    if (options.importTasks && hasTaskData) {
      msg = t('tasksImported', { n: importedTasks.length })
    } else if (options.importConfig && settingsManifests.length) {
      msg = t('configImported')
    }
    ...
    const detail = e instanceof Error ? e.message.trim() : String(e).trim()
    useStore.getState().showToast(detail ? t('importFailedWithDetail', { detail }) : t('importFailedUnknown'), 'error')
    return false
  }
}
...
export async function addImageFromUrl(src: string): Promise<void> {
  ...
  if (!blob.type.startsWith('image/')) throw new Error(t('invalidImageFile'))
