...
const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'sb2api-async', 'fal', 'cloudflare', 'siliconflow', 'zhipu', 'volcengine', 'dashscope'])
...
const CLOUDFLARE_PROVIDER: CustomProviderDefinition = {
  id: 'cloudflare',
...
const SILICONFLOW_PROVIDER: CustomProviderDefinition = {
  id: 'siliconflow',
  name: '硅基流动 SiliconFlow',
  baseUrl: 'https://api.siliconflow.cn/v1',
  models: ['Qwen/Qwen-Image', 'Kwai-Kolors/Kolors', 'black-forest-labs/FLUX.1-schnell', 'black-forest-labs/FLUX.1-dev', 'stabilityai/stable-diffusion-3-5-large'],
  submit: {
    ...
    result: {
      ...
      b64JsonPaths: ['images.*.b64_json'],
    },
  },
}

const ZHIPU_PROVIDER: CustomProviderDefinition = {
  id: 'zhipu',
...
const VOLCENGINE_PROVIDER: CustomProviderDefinition = {
  id: 'volcengine',
  name: '豆包 火山方舟',
...
const DASHSCOPE_PROVIDER: CustomProviderDefinition = {
  id: 'dashscope',
  name: '通义千问 / 万相 DashScope',
  ...
  poll: {
    ...
    result: {
      imageUrlPaths: ['output.results.*.url'],
      b64JsonPaths: [],
    },
  },
}

const PRESET_PROVIDERS: Record<string, CustomProviderDefinition> = {
  cloudflare: CLOUDFLARE_PROVIDER,
  siliconflow: SILICONFLOW_PROVIDER,
  zhipu: ZHIPU_PROVIDER,
  volcengine: VOLCENGINE_PROVIDER,
  dashscope: DASHSCOPE_PROVIDER,
}

function getPresetProvider(provider: ApiProvider): CustomProviderDefinition | null {
...
function normalizeProviderOrder(value: unknown, customProviders: CustomProviderDefinition[]): string[] | undefined {
  ...
  const providerIds = ['openai', 'sb2api-async', 'fal', 'cloudflare', 'siliconflow', 'zhipu', 'volcengine', 'dashscope', ...customProviders.map((provider) => provider.id)]
...
function normalizeSubmitMapping(value: unknown, fallback: CustomProviderSubmitMapping): CustomProviderSubmitMapping {
  ...
  return {
    ...
    headers: normalizeStringRecord(record.headers) ?? fallback.headers,
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (provider === 'cloudflare') {
    return {
      ...
      model: savedDraft?.model ?? 'flux-1-schnell',
      imageGenerationModel: savedDraft?.imageGenerationModel ?? profile.imageGenerationModel,
      apiMode: 'images',
      reasoningEffort: savedDraft?.reasoningEffort,
      codexCli: false,
      ...
      apiKey: profile.provider === 'cloudflare' ? profile.apiKey : (profile.apiKey || 'cloudflare'),
      providerDrafts,
    }
  }
  ...
  if (customProvider) {
    ...
    return {
      ...
      baseUrl: savedDraft?.baseUrl ?? customProvider.baseUrl ?? (shouldUseOpenAIDefaults ? DEFAULT_BASE_URL : profile.baseUrl || DEFAULT_BASE_URL),
      model: savedDraft?.model ?? customProvider.models?.[0] ?? (shouldUseOpenAIDefaults ? DEFAULT_IMAGES_MODEL : profile.model || DEFAULT_IMAGES_MODEL),
      imageGenerationModel: savedDraft?.imageGenerationModel ?? profile.imageGenerationModel,
      apiMode: 'images',
      reasoningEffort: savedDraft?.reasoningEffort,
...
export function normalizeApiProfile(
  ...
  const preset = getPresetProvider(provider)
  const providerFallback = customProviderIds.has(provider) || preset
  ...
  const rawBaseUrl = preset?.baseUrl
    ? (typeof record.baseUrl === 'string' && record.baseUrl.trim() ? record.baseUrl : preset.baseUrl)
  ...
  const streamImages = provider === 'openai'
    ...
    : false
  const nativeTransparentBackgroundUnavailable = (customProviderIds.has(provider) || Boolean(preset)) && !nativeTransparentProviderIds.has(provider)
  ...
  return {
    ...
    apiKey: provider === 'cloudflare'
      ? (typeof record.apiKey === 'string' && record.apiKey.trim() ? record.apiKey : 'cloudflare')
      : typeof record.apiKey === 'string' ? record.apiKey : defaults.apiKey,
    model: typeof record.model === 'string' && record.model.trim()
      ? record.model
      : preset?.models?.[0] ?? defaults.model,
    ...
    apiProxy: provider === 'sb2api-async' || preset ? false : typeof record.apiProxy === 'boolean' ? record.apiProxy : defaults.apiProxy,
...
function validateImportedProfileRecord(input: unknown) {
  ...
  if (baseUrl && (baseUrl.startsWith('[') || baseUrl.includes(']('))) {
    throw new Error(t('jsonContainsMarkdownLink'))
  }

  if (typeof input.apiMode === 'string' && input.apiMode !== 'images' && input.apiMode !== 'responses') {
    throw new Error(t('invalidApiModeFormat'))
  }
}
...
function validateDeploymentProviderIds(input: unknown) {
  ...
  for (const item of input) {
    if (!isRecord(item)) throw new Error(t('deploymentProviderMustBeObject'))
    const name = typeof item.name === 'string' ? item.name.trim() : ''
    const label = name ? `「${name}」` : ''
    const id = typeof item.id === 'string' ? item.id.trim() : ''
    if (!id) throw new Error(t('deploymentProviderMissingId', { label }))
    if (BUILT_IN_PROVIDER_IDS.has(id)) throw new Error(t('deploymentProviderIdConflict', { label, id }))
    if (usedIds.has(id)) throw new Error(t('deploymentProviderIdDuplicate', { label, id }))
    if (!normalizeCustomProviderDefinition(item)) throw new Error(t('deploymentProviderInvalidManifest', { label }))
    usedIds.add(id)
  }
}
...
function normalizeDeploymentProfileEntries(input: unknown, customProviderIds: Set<string>, nativeTransparentProviderIds = new Set<string>(), deploymentConfig = false) {
  ...
  if (deploymentConfig && records.length > 1) {
    ...
    if (defaultCount !== 1) throw new Error(t('deploymentMultipleDefaults'))
  }
  ...
  for (const record of records) {
    ...
    if (id) {
      if (usedIds.has(id)) {
        ...
        const label = name ? `「${name}」` : ''
        throw new Error(t('apiConfigDuplicateId', { label, id }))
      }
      usedIds.add(id)
      continue
    }
...
function validateCustomProviderTaskMappings(providers: CustomProviderDefinition[]) {
  ...
  if (invalid) throw new Error(t('customProviderTaskIdPathNeedsPoll', { name: invalid.name }))
}
...
export function importCustomProviderSettingsFromJson(
  ...
  if (!parsed || typeof parsed !== 'object') {
    throw new Error(t('jsonRootMustBeObject'))
  }
  ...
  if (Array.isArray(record.customProviders)) {
    ...
    if (customProviders.length === 0) {
      if (!options.deploymentConfig) throw new Error(t('customProvidersNoValid'))
    }
  ...
  throw new Error(t('unrecognizedJson'))
}
...
export function validateApiProfile(profile: ApiProfile): string | null {
  if (!profile.name.trim()) return t('missingName')
  if (profile.provider !== 'fal' && !profile.baseUrl.trim() && !shouldUseApiProxy(profile.apiProxy)) return t('missingApiUrl')
  if (!profile.apiKey.trim()) return t('missingApiKey')
  if (!profile.model.trim()) return t('missingModelId')
  return null
}
