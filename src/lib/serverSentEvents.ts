...
export function throwIfAborted(...signals: Array<AbortSignal | undefined>) {
  ...
  if (!signal) return
  throw signal.reason instanceof Error ? signal.reason : new DOMException(t('requestStopped'), 'AbortError')
}
...
export async function readJsonServerSentEvents(
  ...
  if (!response.body) throw new Error(t('streamNotReadable'))
  ...
  try {
    ...
    if (!hasDataLine) {
      const message = t('noValidDataEvents')
