...
export function getExportZipPlan(
  ...
  for (let index = 0; index < plannedTasks.length; index++) {
    addItem(taskBytes[index], t('taskTooLargeForBackup'), (part) => part.tasks.push(plannedTasks[index]))
  }
  for (let index = 0; index < plannedConversations.length; index++) {
    addItem(conversationBytes[index], t('taskTooLargeForBackup'), (part) => part.agentConversations.push(plannedConversations[index]))
  }
  for (const image of plannedImages) {
    addItem(image.bytes, t('imageTooLargeForBackup', { id: image.id }), (part) => part.imageIds.push(image.id))
  }
  return parts
}
...
export async function readExportZipManifest(bytes: Uint8Array, validateFiles = true): Promise<ExportData> {
  ...
  if (!manifestBytes) throw new Error(t('zipMissingManifest'))
...
function assertExportZipFiles(manifest: ExportData, hasFile: (path: string) => boolean) {
  ...
  if (missingPath) throw new Error(t('zipMissingPath', { path: missingPath }))
}
