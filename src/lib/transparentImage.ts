...
export async function removeKeyedBackgroundFromDataUrl(
  ...
  if (!ctx) throw new Error(t('transparentCanvasUnsupported'))
...
export function removeKeyedBackgroundFromPixels(
  ...
  if (data.length < width * height * 4) throw new Error(t('transparentPixelSizeMismatch'))
...
function getKeyColorRgb(keyColor: string): Rgb {
  ...
  if (!rgb) throw new Error(t('transparentKeyColorUnsupported'))
  return rgb
}
