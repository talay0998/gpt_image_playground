...
function markToolStatusStopped(status: AgentWebSearchStatus): AgentWebSearchStatus {
  ...
  return { text: status.stoppedText ?? status.text, completed: true }
}
    ...
    if ((round?.status === 'running' || roundInterrupted) && batchImageCall) {
      blocks.push({
        ...
        status: roundInterrupted
          ? markToolStatusStopped({ text: t('wsFillingBatchParams'), completed: false, stoppedText: t('wsFillingBatchParamsStopped') })
          : { text: t('wsFillingBatchParams'), completed: false },
