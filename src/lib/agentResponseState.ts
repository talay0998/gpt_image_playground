...
export function getAgentRecoveredFailureError(round: AgentRound, tasks: TaskRecord[]) {
  ...
  if (failedTasks.length === 0) return t('imageGenerationFailed')
  if (failedTasks.length === 1) return failedTasks[0].error || t('imageGenerationFailed')
  return t('partialImageTasksFailed')
}
