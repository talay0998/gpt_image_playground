...
function getStreamEventErrorMessage(event: Record<string, unknown>): string | null {
  ...
  if (type?.endsWith('.failed')) return getStringValue(event, 'message') ?? t('agentStreamRequestFailed')
  return null
}
...
async function parseAgentStreamResponse(
  ...
  if (!payload) throw new Error(t('agentStreamNoFinalResponse'))
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    if (!payload) throw new Error(t('agentApiInvalidFormat'))
...
export async function callAgentConversationTitleApi(opts: {
  ...
  try {
    ...
    if (!payload) throw new Error(t('agentTitleApiInvalidFormat'))
  ...
  try {
    ...
    if (profile.streamImages && isEventStreamResponse(response)) {
      ...
      return {
        ...
        error: completedImage ? null : t('streamNoImage'),
    ...
    // Non-streaming
    const payload = normalizeResponsePayload(await response.json())
    if (!payload) throw new Error(t('imageApiInvalidFormat'))
    const images = extractImages(payload, mime)
    ...
    return {
      batchItemId,
      image,
      error: image ? null : t('apiNoImageData'),
      rawResponsePayload: JSON.stringify(payload, null, 2),
    }
  } catch (err) {
    if (controller.signal.aborted || signal?.aborted) {
      return { batchItemId, image: null, error: t('requestCancelled') }
    }
