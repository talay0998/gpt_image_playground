...
function assertMaxBytes(label: string, bytes: number, maxBytes: number) {
  if (bytes > maxBytes) {
    throw new Error(t('payloadTooLarge', { label, size: formatMiB(bytes), max: formatMiB(maxBytes) }))
  }
}

export function assertImageInputPayloadSize(bytes: number) {
  assertMaxBytes(t('imageInputPayloadTooLarge'), bytes, MAX_IMAGE_INPUT_PAYLOAD_BYTES)
}
...
export function maybeAppendTransparentBackgroundHint(message: string): string {
  ...
  return `${message}\n${t('transparentBackgroundUnsupportedHint')}`
}
...
export async function fetchImageUrlAsDataUrl(url: string, fallbackMime: string, signal?: AbortSignal): Promise<string> {
  ...
  try {
    ...
    if (err instanceof TypeError) {
      ...
      if (probe === 'opaque') {
        throw new Error(`${t('imgDownloadCorsFailed')}${getImageFetchCorsHint()}`)
      }
      if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        throw new Error(`${t('imgDownloadNetwork')}${getImageFetchCorsHint()}`)
      }
      throw new Error(`${t('imgDownloadGeneric')}${getImageFetchCorsHint()}`)
    }
    throw err
  }

  if (!response.ok) {
    throw new Error(t('imgDownloadHttpFailed', { status: response.status }))
  }
