    ...
    <div className="fixed inset-0 z-[100] bg-white/60 dark:bg-gray-900/60 backdrop-blur-md flex flex-col items-center justify-center pointer-events-none">
      <div className="flex flex-col items-center gap-4 p-8 rounded-3xl">
        ...
        <div className="text-center">
          {atImageLimit ? (
            <>
              <p className="text-lg font-semibold text-red-500">{t('uploadLimitReached', { n: maxImages })}</p>
              <p className="text-sm text-gray-400 mt-1">{t('removeRefsFirst')}</p>
            </>
          ) : (
            <>
              <p className="text-lg font-semibold text-gray-700 dark:text-gray-200">{t('releaseToUpload')}</p>
