...
export default function InputBar() {
  ...
  }, [defaultFavoriteCollectionId, favoriteCollections, tasks, uiLang])
  ...
  const handleDeleteSelected = useCallback(() => {
    setConfirmDialog({
      title: t('batchDelete'),
      message: t('batchDeleteTasksMsg', { n: selectedTaskIds.length }),
      action: () => {
  ...
  const handleDownloadSelected = useCallback(async () => {
    ...
    if (imageIds.length === 0) {
      showToast(t('selectedTasksNoImages'), 'info')
      return
    }
    ...
    try {
      ...
      if (successCount === 0) {
        ...
        showToast(t('partialDownloadFailed', { ok: successCount, fail: failCount }), 'error')
      } else {
        showToast(successCount > 1 ? t('downloadSuccessN', { n: successCount }) : t('downloadSuccess'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('downloadFailed'), 'error')
  ...
  const handleDownloadSelectedFavoriteCollections = useCallback(async () => {
    ...
    try {
      ...
      if (successCount === 0) {
        showToast(t('selectedCollectionsNoImages'), 'info')
      } else if (failCount > 0) {
        showToast(t('partialDownloadFailed', { ok: successCount, fail: failCount }), 'error')
      } else {
        showToast(useZipDownload && downloadedCollectionCount > 1 ? t('downloadSuccessZips', { zips: downloadedCollectionCount, n: successCount }) : t('downloadSuccessN', { n: successCount }), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('downloadFailed'), 'error')
  ...
  const handleDeleteSelectedFavoriteCollections = useCallback(() => {
    ...
    if (selectedCollections.length === 0) {
      showToast(t('noCollectionsToDelete'), 'info')
      return
    }
    if (favoriteCollections.length - selectedCollections.length < 1) {
      showToast(t('keepAtLeastOneCollection'), 'error')
      return
    }
    ...
    setConfirmDialog({
      title: t('batchDeleteCollections'),
      message: t('batchDeleteCollectionsMsg', { n: selectedCollections.length }),
      checkbox: imageCount > 0
        ? {
            label: t('deleteFavoriteCollectionImages', { n: imageCount }),
            tone: 'danger',
          }
        : undefined,
  ...
  const submitButtonAriaLabel = activeAgentIsRunning
    ? t('stopGenerate')
    : hasSubmitApiConfig
    ? maskDraft ? t('maskEdit') : t('submitGenerate')
    : t('needApi')
  const submitTooltipText = activeAgentIsRunning ? t('stopGenerate') : t('apiNeedTooltip')
  const promptPlaceholder = t('promptPlaceholder')
  ...
  const nLimitHintText = agentAutoImageCount
    ? t('agentCountAutoHint')
    : isFalProvider
    ? t('falMaxRequestCount', { n: outputImageLimit })
    : t('openaiMaxRequestCount', { n: outputImageLimit })
  ...
  const uploadImageTooltipText = atImageLimit ? t('addReferenceLimit', { n: API_MAX_IMAGES }) : t('uploadImage')
    ...
    try {
      ...
      if (currentCount >= API_MAX_IMAGES) {
        useStore.getState().showToast(
          t('addReferenceLimit', { n: API_MAX_IMAGES }),
          'error',
        )
        return
      }
      ...
      if (discarded > 0) {
        useStore.getState().showToast(
          t('uploadLimitDiscarded', { n: API_MAX_IMAGES, discarded }),
          'error',
        )
      }
    } catch (err) {
      useStore.getState().showToast(
        t('imageAddFailed', { msg: err instanceof Error ? err.message : String(err) }),
        'error',
      )
    }
  }
    ...
    const handleDrop = (e: DragEvent) => {
      ...
      if (imageIds.length > 0) {
        Promise.all(imageIds.map(async (imageId) => {
          ...
          if (!dataUrl) {
            showToast(t('someImagesMissing'), 'error')
            return
          }
          addInputImage({ id: imageId, dataUrl })
        })).then(() => {
          showToast(t('imagesUploaded'), 'success')
        }).catch((err) => showToast(t('imageUploadFailed', { msg: err instanceof Error ? err.message : String(err) }), 'error'))
      }
    }
  ...
  const renderImageThumb = (img: (typeof inputImages)[number], idx: number) => {
    ...
    const imageHintText = isMaskTarget ? t('maskMustBeFirst') : ''
      ...
      >
        ...
        >
          ...
          <button
            ...
            title={t('viewLabel')}
            aria-label={t('viewReferenceImage')}
          >
  ...
  const renderClearAllButton = () => (
    <button
      onClick={() =>
        setConfirmDialog({
          title: maskTargetImage ? t('clearAllInputImages') : t('clearReferenceImages'),
          message: maskTargetImage
            ? t('clearMaskTargetConfirm', { n: referenceImages.length })
            : t('clearAllRefsConfirm', { n: inputImages.length }),
      ...
      title={maskTargetImage ? t('clearMaskTargetTitle') : t('clearAllRefsTitle')}
    >
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
      </svg>
      <span className="text-[8px] leading-none">{maskTargetImage ? t('clearAll') : t('clear')}</span>
    </button>
  )
              ...
              <>
                ...
                {mobileCollapsed && (
                  <div className="text-xs text-gray-400 dark:text-gray-500 mb-2 ml-1">
                    {maskDraft ? t('maskTargetCountText', { n: referenceImages.length }) : t('referenceCountText', { n: inputImages.length })}
                  </div>
                )}
              </>
            ) : (
          ...
          <div className={`relative grid${promptExpanded ? ' min-h-0 flex-1' : ''}`}>
            {showAtImageMenu && (
              <div style={{ left: `${menuLeft}px` }} className="absolute bottom-full z-50 mb-2 w-64 overflow-hidden rounded-2xl border border-gray-200/70 bg-white/95 p-1.5 shadow-xl ring-1 ring-black/5 backdrop-blur-xl dark:border-white/[0.08] dark:bg-gray-900/95 dark:ring-white/10">
                <div className="px-2 pb-1 pt-0.5 text-[11px] text-gray-400 dark:text-gray-500">{t('selectImageReference')}</div>
                <div className="max-h-56 overflow-y-auto custom-scrollbar">
                  {atImageOptions.map((option, optionIndex) => (
                    ...
                    >
                      ...
                      {option.type === 'agent-output' && <span className="shrink-0 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-white/[0.06] dark:text-gray-400">{t('historyTag')}</span>}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {prompt.length > 0 && (
              ...
              >
                <ButtonTooltip visible={clearPromptHover} text={t('clearText')} />
                <button
                  type="button"
                  onClick={() => {
            ...
            {showPromptExpand && (
              ...
              >
                <ButtonTooltip visible={expandPromptHover} text={promptExpanded ? t('collapseInput') : t('expandInput')} />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  aria-label={promptExpanded ? t('collapseInput') : t('expandInput')}
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  >
                    ...
                    {activeAgentIsRunning ? t('stopGenerate') : maskDraft ? t('maskEdit') : t('submitGenerate')}
                  </button>
                </div>
              </div>
            </div>
