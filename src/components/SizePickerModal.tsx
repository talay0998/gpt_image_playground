...
export default function SizePickerModal({ currentSize, onSelect, onClose, allowAuto = true, codexCli = false }: Props) {
  ...
  const sizeLimitText = codexCli ? t('codexCliSizeLimit') : t('sizeLimit')
    ...
    >
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">{t('sizePickTitle')}</h3>
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">{t('sizePickCurrent', { size: currentSize || 'auto' })}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="h-[380px] max-h-[55vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10 pr-1 -mr-1 pb-2">
            {mode === 'auto' && (
              <div className="flex h-full animate-fade-in items-center justify-center pt-8 pb-4 text-center">
                <div>
                  ...
                  <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">{t('sizeAutoTitle')}</h4>
              ...
              <div className="space-y-5 animate-fade-in">
                ...
                {ratio === 'custom' && (
                  <label className="block animate-fade-in">
                    <span className="mb-2 block text-xs font-medium text-gray-400 dark:text-gray-500">{t('customRatioInput')}</span>
                    <input
                      value={customRatio}
            ...
            {mode === 'resolution' && (
              <div className="space-y-5 animate-fade-in">
                <section>
                  <div className="mb-4 text-xs font-medium text-gray-400 dark:text-gray-500">{t('pixelInput')}</div>
                  <div className="flex items-center gap-4">
                    <label className="flex-1">
                      <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">{t('widthLabel')}</span>
                      <input
                        type="number"
                        value={customW}
                    ...
                    <label className="flex-1">
                      <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">{t('heightLabel')}</span>
                      <input
                        type="number"
                        value={customH}
