...
export function FavoriteCollectionsView() {
  ...
  const cards = useMemo<CollectionCard[]>(() => {
    ...
    return [
      { id: ALL_FAVORITES_COLLECTION_ID, name: t('statusAll'), tasks: allTasks },
  ...
  }, [collections, defaultFavoriteCollectionId, tasks, uiLang])
    ...
    <div data-favorite-collections-root className="relative min-h-[50vh]">
      {filteredCards.length === 0 ? (
        <div className="py-32 text-center text-gray-400 dark:text-gray-500">
          ...
          <p className="text-sm">{cards.length === 0 ? t('noFavoritedImages') : t('noMatchingCollection')}</p>
        </div>
      ) : (
