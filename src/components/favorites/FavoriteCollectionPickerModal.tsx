...
export function FavoriteCollectionPickerModal() {
  ...
  const handleDelete = (e: React.MouseEvent, collection: FavoriteCollection) => {
    ...
    const collectionTasks = tasks.filter(task => getTaskFavoriteCollectionIds(task, defaultFavoriteCollectionId).includes(collection.id))
    const imageCount = new Set(collectionTasks.flatMap((task) => task.outputImages || [])).size
    setConfirmDialog({
      title: t('deleteFavoriteCollection'),
      message: t('deleteFavoriteCollectionMsg', { name: collection.name }),
  ...
  const handleSetDefault = (e: React.MouseEvent, collection: FavoriteCollection) => {
    ...
    setConfirmDialog({
      title: t('changeDefaultCollection'),
      message: t('changeDefaultCollectionMsg', { from: current.name, to: collection.name }),
    ...
    <div data-no-drag-select className="fixed inset-0 z-[105] flex items-center justify-center p-4 sm:p-0" onClick={closePicker}>
      ...
      <div ref={modalRef} className="relative z-10 flex max-h-[85vh] w-full max-w-[400px] flex-col overflow-hidden rounded-3xl bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-white/50 dark:border-white/[0.08] shadow-[0_8px_40px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_40px_rgb(0,0,0,0.4)] ring-1 ring-black/5 dark:ring-white/10 animate-modal-in" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-4 shrink-0 relative border-b border-gray-100 dark:border-[#333]">
          <FavoriteActionButton tooltip={t('close')} onClick={closePicker} wrapperClassName="absolute right-5 top-5 inline-flex" className="shrink-0 rounded-full p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-white/[0.06] dark:hover:text-gray-200">
          ...
          <h2 className="mb-2 pr-8 flex items-center gap-2.5 text-lg font-semibold text-gray-800 dark:text-gray-100 leading-snug">
            ...
            {t('saveToCollectionTitle')}
          </h2>
          <p className="text-[13px] text-gray-500 dark:text-gray-400 leading-relaxed">
            {t('saveToCollectionDesc')}
          </p>
        </div>
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden pt-3 pb-1">
          <div className="flex items-center justify-between mb-1.5 px-6 shrink-0">
            <span className="text-[13px] font-medium text-gray-500 dark:text-gray-400">{t('selectCollectionsLabel')}</span>
            <div className="flex gap-4">
              <button type="button" onClick={() => setCheckedIds(selectableCollections.map((collection) => collection.id))} className="text-[13px] font-medium text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 transition-colors">{t('selectAll')}</button>
              <button type="button" onClick={() => setCheckedIds([])} className="text-[13px] font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 transition-colors">{t('cancel')}</button>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto custom-scrollbar relative">
            {selectableCollections.length === 0 ? (
              <div className="py-8 text-center text-sm text-gray-400 dark:text-gray-500">{t('noCollections')}</div>
              ...
              >
                ...
                <div className={`flex shrink-0 items-center justify-end gap-2 overflow-hidden pr-4 transition-all duration-150 ${editingId === collection.id ? 'w-12' : 'w-28'}`}>
                    {editingId === collection.id ? (
                      ...
                      <>
                        <FavoriteActionButton tooltip={isDefault ? t('unsetDefaultCollection') : t('setAsDefaultCollection')} onClick={(e) => handleSetDefault(e, collection)} className={`p-1.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded-md transition-colors ${isDefault ? 'text-yellow-500 dark:text-yellow-400' : 'text-gray-400 hover:text-yellow-500 dark:hover:text-yellow-400'}`}><FavoriteIcon filled={isDefault} className="w-3.5 h-3.5" /></FavoriteActionButton>
                        <FavoriteActionButton tooltip={t('rename')} onClick={(e) => startRename(e, collection)} className="p-1.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded-md text-gray-400 hover:text-gray-700 dark:hover:text-white transition-colors"><EditIcon className="w-3.5 h-3.5" /></FavoriteActionButton>
                        <FavoriteActionButton tooltip={canDelete ? t('delete') : t('keepAtLeastOneCollection')} disabled={!canDelete} onClick={(e) => handleDelete(e, collection)} className={`p-1.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded-md transition-colors ${canDelete ? 'text-gray-400 hover:text-red-500 dark:hover:text-red-400' : 'text-gray-300 dark:text-gray-600 cursor-not-allowed'}`}><TrashIcon className="w-3.5 h-3.5" /></FavoriteActionButton>
                      </>
                    )}
                  </div>
              </div>
        ...
        <div className="border-t border-gray-200 p-6 dark:border-[#333] shrink-0">
          ...
          <div className="mt-5 flex gap-4">
            <button type="button" onClick={closePicker} className="flex-1 rounded-xl border border-gray-200 bg-transparent px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors dark:border-white/10 dark:text-gray-200 dark:hover:bg-white/[0.04]">{t('cancel')}</button>
            <button type="button" onClick={handleConfirm} className="flex-1 rounded-xl bg-blue-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-600 transition-colors shadow-sm border border-transparent">{t('confirm')}</button>
          </div>
        </div>
      </div>
    </div>,
