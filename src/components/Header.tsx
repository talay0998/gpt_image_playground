...
import { setUiLang, t, useUiLang } from '../lib/i18n'
      ...
      <header data-no-drag-select className={`safe-area-top fixed top-0 left-0 right-0 z-40 bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200 dark:border-white/[0.08] transition-transform duration-300 ease-in-out ${appMode === 'agent' && !agentMobileHeaderVisible ? '-translate-y-full sm:translate-y-0' : 'translate-y-0'}`}>
        <div className="safe-area-x safe-header-inner max-w-7xl mx-auto flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <div
              className="relative"
              {...langTooltip.handlers}
            >
              <button
                type="button"
                onClick={() => setUiLang(lang === 'zh' ? 'ug' : 'zh')}
                className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors"
                aria-label={t('switchLang')}
              >
                <span className="text-[13px] font-bold leading-none text-gray-600 dark:text-gray-400">
                  {lang === 'zh' ? 'ئۇ' : '中'}
                </span>
              </button>
              <ViewportTooltip visible={langTooltip.visible} className="whitespace-nowrap">
                {t('switchLang')}
              </ViewportTooltip>
            </div>
            <button
