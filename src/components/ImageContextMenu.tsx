...
export default function ImageContextMenu() {
  ...
  const handleEdit = async (e: React.MouseEvent) => {
    ...
    if (inputImages.length >= 16) {
      showToast(t('addReferenceLimit', { n: 16 }), 'error')
      return
    }
    ...
    try {
      ...
      showToast(t('addedReference'), 'success')
    } catch (err) {
      console.error(err)
      showToast(t('addReferenceFailed', { msg: err instanceof Error ? err.message : String(err) }), 'error')
    }
  }
