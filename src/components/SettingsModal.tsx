...
export default function SettingsModal() {
  ...
  const activeProviderUsesApiUrl = (activeProviderIsOpenAICompatible && !getCustomProviderDefinition(draft, activeProfile.provider)?.baseUrl) || activeProfile.provider === 'fal'
  ...
  const defaultProviderOrder = ['openai', 'sb2api-async', 'fal', 'cloudflare', 'siliconflow', 'zhipu', 'volcengine', 'dashscope', ...draft.customProviders.map(p => p.id)]
  const providerOrder = draft.providerOrder || defaultProviderOrder

  const unorderedProviderOptions = [
    { label: t('providerLabelOpenAiCompatible'), value: 'openai', draggable: true },
    { label: t('providerLabelSb2apiAsync'), value: 'sb2api-async', draggable: true },
    { label: 'fal.ai', value: 'fal', draggable: true },
    { label: t('providerLabelCloudflare'), value: 'cloudflare', draggable: true },
    { label: t('providerLabelSiliconFlow'), value: 'siliconflow', draggable: true },
    { label: t('providerLabelZhipu'), value: 'zhipu', draggable: true },
    { label: t('providerLabelVolcengine'), value: 'volcengine', draggable: true },
    { label: t('providerLabelDashscope'), value: 'dashscope', draggable: true },
    ...draft.customProviders.map((provider) => {
      const actions = [
        ...(!presetConfigOnly && !isPresetProviderLocked(provider.id) ? [{ label: t('edit'), onClick: () => openEditCustomProvider(provider) }] : []),
  ...
  const providerOptions = [
    ...(!presetConfigOnly && !activeProfileLocked
      ? [{ label: t('createCustomProvider'), value: ADD_CUSTOM_PROVIDER_VALUE, variant: 'action' as const }]
      : []),
  ...
  const zipDownloadRouteSummary = enabledZipDownloadRouteCount
    ? t('zipRouteSummaryOn', { n: enabledZipDownloadRouteCount })
    : t('zipRouteSummaryOff')
  ...
  const copyProfileImportUrl = async (profile: ApiProfile, options: ProfileImportUrlOptions) => {
    try {
      ...
      showToast(options.includeApiKey ? t('importUrlWithKeyCopied') : t('importUrlCopied'), 'success')
      setCopyImportUrlProfile(null)
    } catch (err) {
      showToast(getClipboardFailureMessage(t('copyImportUrlFail'), err), 'error')
    }
  }
  ...
  const showNotificationPermissionMessage = (result: Exclude<BrowserNotificationPermissionResult, { ok: true }>) => {
    if (result.reason === 'unsupported') {
      showToast(t('notificationUnsupported'), 'error')
    } else if (result.reason === 'insecure') {
      showToast(t('notificationInsecure'), 'error')
    } else if (result.reason === 'denied') {
      showToast(t('notificationDenied'), 'error')
    } else {
      showToast(t('notificationNotEnabled'), 'info')
    }
  }
  ...
  const toggleTaskCompletionNotification = async () => {
    ...
    if (result.ok) {
      ...
      showToast(t('notificationEnabled'), 'success')
    } else {
  ...
  const showDataTransferBusyToast = () => {
    ...
    showToast(dataTransferMode === 'export' ? t('exportingWait') : t('importingWait'), 'info')
  }
  ...
  const duplicateActiveProfile = () => {
    ...
    const profile: ApiProfile = {
      ...
      isDefault: undefined,
      name: `${activeProfile.name}（${t('copy')}）`,
    }
    const nextDraft = normalizeSettings({
      ...draft,
      profiles: [...draft.profiles, profile],
  ...
  const buildCustomProviderFromJson = () => {
    ...
    if (!provider) throw new Error(t('customProviderInvalid'))
    return provider
  }
  ...
  function confirmDeleteCustomProvider(provider: CustomProviderDefinition) {
    ...
    setConfirmDialog({
      ...
      message: t('deleteProviderMsg', { name: provider.name }),
  ...
  const copyCustomProviderLlmPrompt = async () => {
    try {
      ...
      showToast(t('llmPromptCopied'), 'success')
    } catch (err) {
      showToast(getClipboardFailureMessage(t('copyLlmPromptFail'), err), 'error')
    }
  }
  ...
  const handleCustomProviderJsonPaste = async () => {
    ...
    try {
      ...
      if (imported.profiles.length > 0) {
        ...
        setEditingCustomProviderId(null)
        setCustomProviderImportError(null)
        showToast(shouldReplaceActiveProfile ? t('overrodeEmptyConfig') : switchedToExistingProfile ? t('switchedToExisting') : t('jsonImportedSwitched'), 'success')
        return
      }
      ...
      setCustomProviderImportError(null)
      showToast(t('jsonImported'), 'success')
    } catch (err) {
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              <div className="space-y-4">
                <div>
                  <div className="mb-1.5 flex items-center gap-1.5">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('currentConfig')}</span>
                    <span className="relative inline-flex">
                      <button
                        ...
                        aria-label={t('copyProfileUrlTitle', { name: activeProfile.name })}
                      >
                    ...
                    {!presetConfigOnly && <span className="relative inline-flex">
                      <button
                        ...
                        className="flex h-5 w-5 items-center justify-center rounded-md text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-white/[0.08] dark:hover:text-gray-200"
                        aria-label={t('copyCurrentConfig')}
                      >
                    ...
                    {showProfileMenu && !profileMenuDisabled && (
                      <>
                        ...
                        >
                          ...
                          >
                            <span className="truncate font-semibold">{t('createNewConfig')}</span>
                              ...
                              return (
                                ...
                                <div className="flex shrink-0 items-center gap-1">
                                  <button
                                    ...
                                    aria-label={t('copyProfileUrlTitle', { name: profile.name })}
                                    title={t('copyImportUrl')}
                                  >
                                    <LinkIcon className="h-3.5 w-3.5" />
                                  </button>
                                  {!presetConfigOnly && (isDefaultProfile || draft.profiles.length > 1) && (
                                    <TooltipButton
                                      tooltip={isPresetProfile && presetDeletionPrevented ? t('presetCannotDelete') : t('deleteConfig')}
                                      ...
                                      onClick={(e) => {
                                        ...
                                        setConfirmDialog({
                                          ...
                                          message: t('deleteConfigMsg', { name: profile.name }),
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('configName')}</span>
                <input
              ...
              {/* 2. 服务商类型 */}
              <div className="block">
                <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('providerType')}</span>
                <Select
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('apiUrl')}</span>
                  </div>
                  <input
                  ...
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyEnabled ? (
                      <span className="text-yellow-600 dark:text-yellow-500">{t('apiProxyOnHint')}</span>
                    ) : activeProfile.provider === 'fal' ? (
                      <span>{t('falUrlHintLead')} <code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">{DEFAULT_FAL_BASE_URL}</code>{t('falUrlHintTail')}</span>
                    ) : (
                      <span>{t('slashUrlHintLead')} <code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">/</code> {t('slashUrlHintTail')} <code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">/v1</code> {t('slashUrlQueryHint')}<code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">?apiUrl=</code>。</span>
                    )}
                  </div>
                </label>
              )}
              ...
              {apiProxyAvailable && activeProviderIsOpenAICompatible && !activeCustomProviderAsync && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('apiProxy')}</span>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyLocked ? t('apiProxyLockedHint') : t('apiProxyHint')}
                  </div>
                </div>
              )}

              {/* 5. API Key */}
              {activeProfile.provider !== 'cloudflare' && (
              <div className="block">
                <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('apiKey')}</span>
                ...
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                  {t('apiKeyQueryHint')}<code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">?apiKey=</code>
                </div>
              </div>
              )}
              ...
              {activeProfile.provider === 'openai' && (
                <div className="block">
                  <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('apiInterface')}</span>
                  <Select
                  ...
                  />
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                    {t('apiKeyQueryHint')}<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">apiMode=images</code> {t('or')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">apiMode=responses</code>。
                  </div>
                </div>
              )}
              ...
              <label className="block">
                ...
                {activeCustomProvider?.models?.length ? (
                  <Select
                    value={activeProfile.model}
                    onChange={(value) => updateActiveProfile({ model: String(value) }, true)}
                    options={activeCustomProvider.models.map((model) => ({ label: model, value: model }))}
                    disabled={activeProfileLocked}
                    className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                  />
                ) : (
                  ...
                  {activeProfile.provider === 'fal' ? (
                    <>
                      {t('currentSupported')}<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">openai/gpt-image-2</code>{' '}
                    ...
                    <>{t('currentUseProviderLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{activeCustomProvider.name}</code>{t('currentUseProviderTail')}</>
                  ) : (activeProfile.apiMode ?? DEFAULT_SETTINGS.apiMode) === 'responses' ? (
                    <>{t('responsesModelHintLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">image_generation</code> {t('responsesModelHintMid')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{DEFAULT_RESPONSES_MODEL}</code>。</>
                  ) : (
                    <>{t('imagesModelHintLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{DEFAULT_IMAGES_MODEL}</code>。</>
                  )}
                  {activeProfile.provider === 'openai' && (
                    <>{t('modelQueryHint')}<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">?model=</code>。</>
                  )}
                </div>
              </label>
              ...
              {activeProfile.provider === 'openai' && activeProfile.apiMode === 'responses' && (
                <label className="block">
                  <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('imageGenerationModel')}</span>
                  <input
                  ...
                  />
                  <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                    {t('imageGenModelHintLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">image_generation</code> {t('imageGenModelHintTail')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{DEFAULT_IMAGES_MODEL}</code>。
                    {t('imageGenModelHintTail2')}<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">?imageGenerationModel=</code>。
                  </div>
                </label>
              )}
              ...
              {(activeProfile.apiMode ?? DEFAULT_SETTINGS.apiMode) === 'responses' && activeProfile.provider === 'openai' && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between gap-3">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('reasoningEffort')}</span>
                  ...
                  <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                    {t('reasoningHintLead')}<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">?reasoningEffort=high</code>。
                  </div>
                </div>
              )}
              ...
              {activeProfile.provider === 'openai' && (
                <div className="block space-y-3">
                  <div>
                    <div className="mb-1.5 flex items-center justify-between gap-3">
                      <span className="block text-sm text-gray-600 dark:text-gray-300">{t('streaming')}</span>
                      <button
                        type="button"
                  ...
                  <div className={`block ${activeProfile.streamImages ? '' : 'opacity-60'}`}>
                    <div className="mb-1.5 flex items-center justify-between gap-3">
                      <span className="block text-sm text-gray-600 dark:text-gray-300">{t('streamPartialCount')}</span>
                      <div className="w-28 shrink-0">
                        <Select
                          ...
                          options={[
                            { label: t('zeroNotRequest'), value: 0 },
                            { label: t('imageCount', { n: 1 }), value: 1 },
                            { label: t('imageCount', { n: 2 }), value: 2 },
                            { label: t('imageCount', { n: 3 }), value: 3 },
                          ]}
                          className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-1.5 text-xs text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                        />
                      </div>
                    ...
                    <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                      {t('streamPartialHintLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">partial_images</code> {t('streamPartialHintTail')}
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="block">
                <div className="mb-1.5 flex items-center justify-between gap-3">
                  <span className="block text-sm text-gray-600 dark:text-gray-300">{t('transparentMethod')}</span>
                  <div className="w-28 shrink-0">
                    <Select
                      ...
                      options={[
                        { label: t('apiNative'), value: 'api' },
                        { label: t('localPostProcess'), value: 'local' },
                      ]}
                ...
                <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                  {activeCustomProviderSupportsNativeTransparentBackground
                    ? t('transparentBgApiHint')
                    : <>{t('transparentBgManifestLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">$params.background</code> {t('transparentBgManifestTail')}</>}
                </div>
              </div>
              ...
              {activeProviderIsOpenAICompatible && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('returnBase64')}</span>
                    <button
                      type="button"
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {t('base64HintLead')} <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">response_format</code> {t('base64HintTail')}
                  </div>
                </div>
              )}
              ...
              {activeProviderIsOpenAICompatible && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">{t('codexCliMode')}</span>
                    <button
                      type="button"
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {t('codexCliHint')}<code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">codexCli=true</code>。
                  </div>
                </div>
              )}
              ...
              {activeProviderIsOpenAICompatible && (
                <label className="block">
                  <span className="mb-1.5 block text-sm text-gray-600 dark:text-gray-300">{t('requestTimeout')}</span>
                  <input
                    value={timeoutInput}
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100">{t('exportData')}</h4>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{t('exportHint')}</p>
                  <div className="flex flex-wrap gap-x-6 gap-y-3">
                    <Checkbox
                      checked={exportConfig}
                      onChange={setExportConfig}
                      label={t('includeConfig')}
                    />
                    <Checkbox
                      checked={exportTasks}
                      onChange={setExportTasks}
                      label={t('includeTasksImages')}
                    />
                  </div>
                  <button
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100">{t('importData')}</h4>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{t('importHint')}</p>
                  <div className="flex flex-wrap gap-x-6 gap-y-3">
                ...
                <div className="rounded-2xl border border-red-100/50 bg-red-50/30 p-4 dark:border-red-500/10 dark:bg-red-500/5 space-y-4 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-sm font-bold text-red-500/90 dark:text-red-400">{t('clearData')}</h4>
                  </div>
                  <div className="flex flex-wrap gap-x-6 gap-y-3">
                    <Checkbox
                      checked={clearConfig}
