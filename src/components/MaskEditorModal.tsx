...
function fillWhiteMask(canvas: HTMLCanvasElement) {
  ...
  if (!ctx) throw new Error(t('canvasUnsupported'))
...
function drawMaskImageToCanvas(maskImage: HTMLImageElement, maskCanvas: HTMLCanvasElement) {
  ...
  if (Math.abs(maskAspect - canvasAspect) > 0.001) {
    throw new Error(t('maskSizeMismatch'))
  }

  const maskCtx = maskCanvas.getContext('2d', { willReadFrequently: true })
  if (!maskCtx) throw new Error(t('canvasUnsupported'))
  ...
  useEffect(() => {
    ...
    async function loadCanvases() {
      try {
        ...
        if (cancelled) return
        if (!dataUrl) {
          showToast(t('maskImageMissing'), 'error')
          setMaskEditorImageId(null)
          return
        }
        ...
        if (maskDraft?.targetImageId === targetImageId) {
          try {
            ...
            fillWhiteMask(maskCanvas)
            showToast(
              t('maskDraftLoadFailed', { msg: err instanceof Error ? err.message : String(err) }),
              'error',
            )
          }
        }
        ...
        if (preparedTarget.wasResized) {
          showToast(
            t('maskImageResized', {
              from: `${preparedTarget.originalWidth}×${preparedTarget.originalHeight}`,
              to: `${preparedTarget.width}×${preparedTarget.height}`,
            }),
            'info',
          )
        }
    ...
    <>
      ...
      <div className="flex-none flex items-center justify-between px-4 py-3 sm:px-6 sm:py-3.5 border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 z-20">
        <div className="flex items-center gap-3">
          <button onClick={close} disabled={isSaving} className="p-2 sm:p-2.5 -ml-2 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl dark:text-gray-400 dark:hover:bg-gray-800 transition" title={t('cancel')}>
            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div className="relative flex items-center gap-1.5 sm:gap-2">
            <h2 className="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-200" id="mask-editor-title">{t('maskEditorTitle')}</h2>
            <button
              type="button"
              ...
              aria-label={t('maskEditDescription')}
            >
            ...
            {showMaskInfo && (
              <div className="absolute left-0 top-full mt-2 w-64 sm:w-72 rounded-xl border border-gray-200/80 bg-white px-3.5 py-2.5 text-xs sm:text-sm leading-5 sm:leading-6 text-gray-600 shadow-lg dark:border-white/[0.08] dark:bg-gray-900 dark:text-gray-300">
                ...
                <p>{t('maskInfoLine1')}</p>
                <p className="mt-2">{t('maskInfoLine2')}</p>
              </div>
            )}
          </div>
        </div>
