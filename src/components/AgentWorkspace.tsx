...
export default function AgentWorkspace() {
  ...
  const handleDeleteConversation = (id: string) => {
    ...
    setConfirmDialog({
      ...
      message: t('deleteConversationMsg'),
      checkbox: generatedImageCount > 0
        ? {
            label: t('deleteConversationGeneratedImages', { n: generatedImageCount }),
            tone: 'danger',
          }
        : undefined,
  ...
  const startRenameConversation = (e: ReactMouseEvent | React.TouchEvent, id: string, currentTitle: string) => {
    ...
    if (agentGeneratingTitleIds[id]) {
      showToast(t('titleGenerating'), 'info')
      return
    }
  ...
  const handleDeleteMessage = (message: AgentMessage, round: AgentRound) => {
    ...
    setConfirmDialog({
      title: isUserMessage ? t('deleteRound') : t('deleteMessage'),
      ...
      action: async () => {
        ...
        try {
          ...
          if (result === 'running') {
            showToast(t('roundStillGenerating'), 'info')
            return true
          }
          if (result === 'not-found') {
            showToast(isUserMessage ? t('roundNotFound') : t('messageNotFound'), 'info')
            return true
          }
          if (result === 'deleted-with-warning') {
            showToast(t('deleteCleanupWarning'), 'error')
            return true
          }
          showToast(isUserMessage ? t('roundDeleted') : t('messageDeleted'), 'success')
          return true
        } catch (err) {
          console.error(err)
          showToast(isUserMessage ? t('deleteRoundFailed') : t('deleteMessageFailed'), 'error')
          return false
        }
      },
    })
  ...
  const handleReuse = (task: TaskRecord) => {
    setConfirmDialog({
      title: t('switchToGalleryTitle'),
      message: t('switchToGalleryMsg'),
      confirmText: t('switchAndReuse'),
      cancelText: t('cancel'),
      action: () => {
        setAppMode('gallery')
  ...
  const handleCopyMessage = async (content: string, successMessage = t('copyPromptDone'), failureMessage = t('copyPromptFailed')) => {
    try {
  ...
  const handleErrorCopyClick = (e: ReactMouseEvent<HTMLDivElement>, content: string) => {
    ...
    void handleCopyMessage(content, t('copyErrorDone'), t('copyErrorFailed'))
  }
    ...
    >
        ...
        <div className="pl-[max(1rem,env(safe-area-inset-left))] flex h-full min-h-0 w-full flex-col">
          <div className="safe-area-top shrink-0">
            <div className="flex h-14 items-center justify-between gap-2 px-4">
              <button type="button" onClick={() => setSidebarCollapsed(true)} className="lg:hidden p-2 -ml-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 rounded-lg transition-colors" title={t('collapseSidebar')}>
                <SidebarLeftIcon className="w-5 h-5" />
              </button>
              <button type="button" onClick={createConversation} className="p-2 -mr-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 lg:hover:bg-gray-100 lg:dark:hover:bg-white/[0.04] rounded-lg transition-colors" title={t('newChat')}>
          ...
          {filteredConversations.length === 0 && (
            <div className="px-2 py-8 text-center text-sm text-gray-400">{t('noChatMatch')}</div>
          )}
      ...
      <section className="min-w-0 flex-1 flex flex-col relative">
        ...
        <div className={`sticky top-0 z-20 lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${mobileTopBarVisible ? 'max-h-16 opacity-100 mb-2' : 'max-h-0 opacity-0 mb-0 pointer-events-none'}`}>
          ...
          >
            <button type="button" onClick={() => setSidebarCollapsed(false)} className="p-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-white/[0.04] rounded-lg transition-colors" title={t('expandConversationList')}>
              <SidebarLeftIcon className="w-5 h-5" />
            </button>
            <button
              type="button"
            ...
            <button type="button" onClick={createConversation} className="p-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-white/[0.04] rounded-lg transition-colors" title={t('newChat')}>
              <EditIcon className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        >
          {!conversation ? (
            <div className="py-20 text-center text-gray-400">
              <p className="mb-3">{t('noAgentConversation')}</p>
              <button type="button" onClick={createConversation} className="rounded-lg bg-blue-500 px-4 py-2 text-white hover:bg-blue-600 transition-colors">{t('createConversation')}</button>
            </div>
          ) : (
            (() => {
              if (activeMessages.length === 0) {
                return (
                  <div className="py-20 text-center text-gray-400">
                    <p className="mb-2">{t('startAgentConversation')}</p>
                    <p className="text-xs">{t('startAgentConversationHint')}</p>
                  </div>
                )
              }
                  ...
                  <div key={message.id} className={`flex w-full mb-6 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
                    ...
                    <div className="mb-2 flex items-center justify-between gap-4 text-sm text-gray-500 dark:text-gray-400">
                      <span className="font-medium">
                         <span className={isAssistant ? 'text-blue-600 dark:text-blue-400 font-semibold' : 'text-gray-700 dark:text-gray-200 font-semibold'}>{isAssistant ? 'Agent' : t('user')}</span> <span className="opacity-60 font-normal ml-1">{t('roundIndicator', { n: round?.index ?? '?' })}</span>
                      </span>
                    </div>
                              ...
                              return (
                                <div key={block.key} className="mt-4 max-w-sm" onClick={e => e.stopPropagation()}>
                                  <TaskCard
                                    ...
                                    onDelete={() => setConfirmDialog({ title: t('deleteTask'), message: t('deleteTaskMsgPlain'), action: () => removeTask(block.task) })}
                                  />
                                </div>
                              )
                      ...
                      <div className="flex min-w-0 items-center gap-2">
                        {isEditing && (
                          <div className="inline-flex items-center rounded-md bg-blue-100 px-2 py-1 text-xs text-blue-700 dark:bg-blue-500/20 dark:text-blue-300">
                            <span className="truncate">{t('editing')}</span>
                            <AgentActionButton
                              tooltip={t('cancelEditing')}
                      ...
                      <div className="flex items-center gap-2 ml-auto text-gray-400">
                        {!isAssistant && round && hasBranches && siblingIndex >= 0 && (
                          <div className="inline-flex items-center text-sm font-bold text-gray-400 dark:text-gray-500 mr-1">
                            <AgentActionButton tooltip={t('prevBranch')} className="p-1 rounded-md hover:bg-gray-200/50 dark:hover:bg-white/10 hover:text-gray-800 dark:hover:text-gray-200 transition-colors" onClick={() => handleSwitchBranch(round, -1)}>
                              <ChevronLeftIcon className="w-4 h-4" />
                            </AgentActionButton>
                            <span className="px-1 tabular-nums tracking-widest">{siblingIndex + 1}/{siblingRounds.length}</span>
                            <AgentActionButton tooltip={t('nextBranch')} className="p-1 rounded-md hover:bg-gray-200/50 dark:hover:bg-white/10 hover:text-gray-800 dark:hover:text-gray-200 transition-colors" onClick={() => handleSwitchBranch(round, 1)}>
                        ...
                        {isAssistant ? (
                          <>
                            <AgentActionButton tooltip={t('copyOutputText')} className={`p-1.5 rounded-md transition-colors ${message.content.trim() ? 'text-gray-400 hover:text-gray-700 hover:bg-gray-100 dark:hover:text-gray-200 dark:hover:bg-white/[0.06]' : 'text-gray-300 dark:text-gray-600 opacity-50 cursor-not-allowed'}`} disabled={!message.content.trim()} onClick={() => {
                              void handleCopyMessage(getAgentAssistantCopyContent(message.content, assistantBlocks), t('outputCopied'), t('copyOutputFailed'));
                            }}>
                              <CopyIcon className="w-4 h-4" />
                            </AgentActionButton>
                            <AgentActionButton tooltip={t('regenerate')} className="p-1.5 rounded-md text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors" onClick={() => {
                              if (conversation && round) void regenerateAgentAssistantMessage(conversation.id, round.id);
                            }}>
                              <RefreshIcon className="w-4 h-4" />
                            </AgentActionButton>
                            <AgentActionButton tooltip={allRoundTasksFavorited ? t('editFavorite') : t('favoriteAllImages')} className={`p-1.5 rounded-md transition-colors ${hasRoundFavoriteTasks ? (allRoundTasksFavorited ? 'text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10' : 'text-gray-400 hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10') : 'text-gray-300 dark:text-gray-600 opacity-50 cursor-not-allowed'}`} disabled={!hasRoundFavoriteTasks} onClick={() => {
                            ...
                            <AgentActionButton tooltip={t('downloadAllImages')} className={`p-1.5 rounded-md transition-colors ${hasRoundTasks ? 'text-gray-400 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10' : 'text-gray-300 dark:text-gray-600 opacity-50 cursor-not-allowed'}`} disabled={!hasRoundTasks} onClick={async () => {
                               ...
                               try {
                                 ...
                                 if (successCount === 0) {
                                   useStore.getState().showToast(t('downloadFailed'), 'error');
                                 } else if (failCount > 0) {
                                   useStore.getState().showToast(t('partialDownloadFailed', { ok: successCount, fail: failCount }), 'error');
                                 } else {
                                   useStore.getState().showToast(successCount > 1 ? t('downloadSuccessN', { n: successCount }) : t('downloadSuccess'), 'success');
                                 }
                               } catch (err) {
                                 console.error(err);
                                 useStore.getState().showToast(t('downloadFailed'), 'error');
                            ...
                            <AgentActionButton tooltip={t('deleteMessage')} className="p-1.5 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors" onClick={() => {
                          ...
                          <>
                            <AgentActionButton tooltip={t('copyPrompt')} className="p-1.5 rounded-md hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-200/50 dark:hover:bg-white/[0.04] transition-colors" onClick={() => {
                              void handleCopyMessage(message.content);
                            }}>
                              <CopyIcon className="w-4 h-4" />
                            </AgentActionButton>
                            <AgentActionButton tooltip={t('edit')} className="p-1.5 rounded-md hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-200/50 dark:hover:bg-white/[0.04] transition-colors" onClick={() => {
                               if (round) void handleEditRoundMessage(round, message.content);
                            }}>
                              <EditIcon className="w-4 h-4" />
                            </AgentActionButton>
                            <AgentActionButton tooltip={t('delete')} className="p-1.5 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors" onClick={() => {
                              if (round) handleDeleteMessage(message, round);
                            }}>
                              <TrashIcon className="w-4 h-4" />
                            </AgentActionButton>
                    ...
                    <div key={`running-${round.id}`} className="flex w-full justify-start mb-6">
                      <article className="flex min-w-[16rem] max-w-[95%] flex-col rounded-2xl rounded-tl-sm border border-gray-200 bg-white/70 p-4 dark:border-white/[0.08] dark:bg-white/[0.03] md:max-w-[85%] lg:max-w-[75%]">
                        <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                          <span className="text-blue-600 dark:text-blue-400 font-semibold">Agent</span> <span className="ml-1 font-normal opacity-60">{t('roundIndicator', { n: round.index })}</span>
                        </div>
