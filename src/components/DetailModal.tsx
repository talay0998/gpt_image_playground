...
export default function DetailModal() {
  ...
  const taskProviderName = taskProvider ? getApiProviderLabel(settings, taskProvider) : t('unknown')
  ...
  const outputCompressionText = task.params.output_compression == null ? t('notSet') : String(task.params.output_compression)
  ...
  const handleCopyError = async () => {
    const errorText = task.error || t('generationFailed')
    try {
      await copyTextToClipboard(errorText)
      showToast(t('copyErrorDone'), 'success')
    } catch (err) {
  ...
  const handleCopyPrompt = async () => {
    ...
    try {
      ...
      showToast(t('copyPromptDone'), 'success')
    } catch (err) {
  ...
  const handleShowPromptWarning = () => {
    showCodexCliPrompt(
      true,
      currentRevisedPrompt ? t('promptRewritten') : t('promptMissingInfo'),
    )
  }
  ...
  const handleCopyInputImage = async () => {
    ...
    try {
      ...
      showToast(t('copyRefDone'), 'success')
    } catch (err) {
      console.error(err)
      showToast(getClipboardFailureMessage(t('copyRefFailed'), err), 'error')
    }
  }
  ...
  const handleDownloadCurrentOriginalOutput = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(t('origDownloaded'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('downloadFailed'), 'error')
  ...
  const handleDownloadAllOutputs = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(t('partialDownloadFailed', { ok: result.successCount, fail: result.failCount }), 'error')
      } else {
        showToast(result.successCount > 1 ? t('downloadSuccessN', { n: result.successCount }) : t('downloadSuccess'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('downloadFailed'), 'error')
  ...
  const handleDownloadPartialImages = async () => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(t('downloadMiddleDone', { n: result.successCount }), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('downloadFailed'), 'error')
    ...
    >
        ...
        <div className="md:w-1/2 w-full h-64 md:h-auto bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
          ...
          {task.status === 'done' && outputLen > 0 && currentOutputError && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <p className="text-sm font-medium text-red-500">{t('outputIndexFailed', { n: currentOutputSlot.requestIndex + 1 })}</p>
              <p
                      ...
                      onClick={async () => {
                        if (task.rawImageUrls!.length === 1) {
                          ...
                          try {
                            ...
                            showToast(t('copyLinkDone'), 'success')
                          } catch (err) {
                            showToast(getClipboardFailureMessage(t('copyLinkFailed'), err), 'error')
                          }
                        } else {
                          dismissAllTooltips()
        ...
        <div className="md:w-1/2 w-full p-5 overflow-y-auto overscroll-contain flex flex-col">
          ...
          <div data-selectable-text className="flex-1">
            ...
            {showPendingPrompt ? (
              <div className="mb-4 leading-relaxed">
                <p className="text-sm text-gray-700 dark:text-gray-300">{t('pendingPromptTitle')}</p>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{t('pendingPromptHint')}</p>
              </div>
            ) : (
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap mb-4">
                {task.prompt || t('noPromptText')}
              </p>
            )}
              ...
              <div className="mb-4">
                ...
                {isAgentTask && (
                  <div className={`${allInputImageIds.length > 0 ? 'mt-2 ' : ''}text-xs text-gray-500 dark:text-gray-400`}>
                    {allInputImageIds.length > 0 ? t('agentRefImages') : t('agentRefAuto')}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2 text-xs mb-4 min-w-0">
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{t('paramSize')}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{t('paramQuality')}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{t('paramFormat')}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {supportsTransparentOutput && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{t('transparentBg')}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {task.params.output_format !== 'png' && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{t('compressionLabel')}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{t('moderationLabel')}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {!isAgentTask && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{t('paramCount')}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mb-4">
              <span>{t('createdAtText', { time: formatTime(task.createdAt) })}</span>
              {(() => {
                const duration = formatDuration()
                if (!duration) return null
                return <span> · {t('durationText', { duration })}</span>
              })()}
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 sm:flex gap-2 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
            ...
            <button
              ...
              title={task.isFavorite ? t('editFavorite') : t('favoriteTask')}
            >
        ...
        >
          <div ref={rawUrlsModalRef} className="flex w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#1c1c1e]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('rawUrlsTitle', { n: rawImageUrls.length })}</h3>
      ...
      {showRawResponseModal && task?.rawResponsePayload && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('rawResponseTitle')}</h3>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={async () => {
