...
function formatTime(value: number) {
  ...
  if (time >= startOfToday) return t('timeToday')
  if (time >= startOfYesterday) return t('timeYesterday')
  if (time >= startOfWeek) return t('timeThisWeek')
  return t('timeEarlier')
}
      ...
      checkbox: relatedTaskIds.length > 0
        ? {
            label: generatedImageCount > 0
              ? t('deleteConversationWithImages', { n: generatedImageCount })
              : t('deleteConversationWithTasks', { n: relatedTaskIds.length }),
            tone: 'danger',
          }
        : undefined,
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-white/10 shrink-0">
        ...
        />
        <HistoryActionButton tooltip={t('close')} onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg text-gray-500 dark:text-gray-400 transition-colors">
          <CloseIcon className="w-4 h-4" />
        </HistoryActionButton>
      </div>
      <div className="flex-1 overflow-y-auto p-2 space-y-1 overscroll-contain">
        {filteredConversations.length === 0 && (
          <div className="px-3 py-8 text-center text-sm text-gray-500">{t('noChatMatch')}</div>
        )}
